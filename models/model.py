...
class my_model(nn.Module):
    def __init__(self, Ch, stages, sf):  # ch：通道数，stages：迭代次数默认4，sf：放大倍数
        ...
        # reconstruct_model
        #残差块数量n_resblocks、特征通道数n_feats、上采样比例upscale_factor、输入图像通道数n_colors、残差块缩放比例res_scale
    ...
    def forward(self, Y, X):  # Y: RGB  X: LR
        ...
        for i in range(0, self.s):
            ZtR = self.R(Zt)#将高光谱降维到多光谱，然后进行操作
            Res1 = self.RT(ZtR - Y)
            BZt = self.B(Zt)
            Res2 = self.BT(BZt - X)
            feat = self.conv(torch.cat((Zt, Y), 1))#沿着通道维度进行拼接，(batch_size, channels, height, width)
            if i == 0:
                re_list.append(feat)
...
if __name__ == '__main__':  # 记得注释forward第一行
    ...
    if 0:
        ...
        x = torch.randn(1, 31, 64, 64).cuda()
        y = torch.randn(1, 3, 512, 512).cuda()
        model = my_model(Ch=31, stages=4, sf=8).cuda()
    z = model(y, x)
    print(z.shape)
